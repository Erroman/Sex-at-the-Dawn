WComments = {
  ...
  auth: function () {
    var
      ...
      var active = this.active = window.open('/login.php?app=-1&layout=widgets', 'vk_openapi', features);
      function checkWnd() {
         if (active.closed) {
...
function showCaptchaBox (sid, dif, box, o) {
  ...
  cur.Rpc.callMethod('showBox', 'captcha.php?' + ajx2q({act: 'show_captcha_box', sid: sid, src: imgSrc, need_mobile: window.need_mobile_act == 1 ? 1 : 0, widget: 1, widget_width: 322}), {height: window.outerHeight || screen.availHeight || 768, width: window.outerWidth || screen.availWidth || 1028});
